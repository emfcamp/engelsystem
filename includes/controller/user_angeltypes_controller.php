...
function user_angeltypes_unconfirmed_hint() {
  ...
  return info(sprintf(ngettext("There is %d unconfirmed role.", "There are %d unconfirmed roles.", count($unconfirmed_user_angeltypes)), count($unconfirmed_user_angeltypes)) . " " . _('Roles which need approval:') . ' ' . join(', ', $unconfirmed_links), true);
}
...
function user_angeltypes_delete_all_controller() {
  ...
  if (! User_is_AngelType_coordinator($user, $angeltype)) {
    error(_("You are not allowed to delete all users for this role."));
    redirect(page_link_to('angeltypes'));
  }
  ...
  if (isset($_REQUEST['confirmed'])) {
    ...
    engelsystem_log(sprintf("Denied all users for role %s", AngelType_name_render($angeltype)));
    success(sprintf(_("Denied all users for role %s."), AngelType_name_render($angeltype)));
...
function user_angeltypes_confirm_all_controller() {
  ...
  $angeltype = AngelType($_REQUEST['angeltype_id']);
  if ($angeltype === false)
    engelsystem_error("Unable to load role.");
  if ($angeltype == null) {
    error(_("Role doesn't exist"));
    redirect(page_link_to('angeltypes'));
  }
  ...
  if ($user_angeltype === false)
    engelsystem_error("Unable to load user role.");
  if ($user_angeltype == null) {
    error(_("User role doesn't exist."));
    redirect(page_link_to('angeltypes'));
  }
  
  if (! in_array('admin_user_angeltypes', $privileges) && ! $user_angeltype['coordinator']) {
    error(_("You are not allowed to confirm all users for this role."));
    redirect(page_link_to('angeltypes'));
  }
  ...
  if (isset($_REQUEST['confirmed'])) {
    ...
    engelsystem_log(sprintf("Confirmed all users for role %s", AngelType_name_render($angeltype)));
    success(sprintf(_("Confirmed all users for role %s."), AngelType_name_render($angeltype)));
    redirect(page_link_to('angeltypes') . '&action=view&angeltype_id=' . $angeltype['id']);
  }
...
function user_angeltype_confirm_controller() {
  ...
  if (! User_is_AngelType_coordinator($user, $angeltype)) {
    error(_("You are not allowed to confirm this users role."));
    redirect(page_link_to('angeltypes'));
  }
  ...
  if (isset($_REQUEST['confirmed'])) {
    ...
    if ($result === false)
      engelsystem_error("Unable to confirm user role.");
    
    engelsystem_log(sprintf("%s confirmed for role %s", User_Nick_render($user_source), AngelType_name_render($angeltype)));
    success(sprintf(_("%s confirmed for role %s."), User_Nick_render($user_source), AngelType_name_render($angeltype)));
    redirect(page_link_to('angeltypes') . '&action=view&angeltype_id=' . $angeltype['id']);
  }
  
  return array(
      _("Confirm role for user"),
...
function user_angeltype_delete_controller() {
  ...
  if ($user['UID'] != $user_angeltype['user_id'] && ! User_is_AngelType_coordinator($user, $angeltype)) {
    error(_("You are not allowed to delete this user's role."));
    redirect(page_link_to('angeltypes'));
  }
  ...
  if (isset($_REQUEST['confirmed'])) {
    ...
    if ($result === false)
      engelsystem_error("Unable to delete user role.");
        ...
        foreach ($users_source as $user_source)
          if ($user_source['UID'] == $user_id) {
            ...
            if ($user_angeltype_id === false)
              engelsystem_error("Unable to create user role.");
